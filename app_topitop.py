...
if uploaded_file:
    try:
        ...
        # Si la data ya es mensual, simplemente la ordena correctamente en el primer día del mes.
        ...
        for nombre, fc in zip(modelos_lista, forecasts_val):
            ...
            rmse = np.sqrt(mean_squared_error(test, fc)) # CÁLCULO DE RMSE AGREGADO
        ...
        # Resaltar en verde el modelo más preciso según el MAPE
        ...
        with st.spinner("Proyectando escenarios futuros con el 100% de la data..."):
            ...
            m_full = Prophet(yearly_seasonality=True, interval_width=0.95).fit(df_p_full) # 95% de nivel de confianza
        ...
        # Cálculos de negocio y protección contra valores ilógicos (negativos)
        ...
        # Stock de seguridad es el buffer para no quebrar inventario si la demanda llega a su tope
        ...
        st.plotly_chart(fig_est, use_container_view=True)
        ...
        # Buffer de memoria para crear el archivo binario
        buffer = io.BytesIO()

        # Usar ExcelWriter de Pandas
        with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
            df_estrategico.to_excel(writer, index=False, sheet_name='Plan_Produccion_Finanzas')
            
            # Auto-ajustar el ancho de las columnas para una apariencia profesional
...
# Footer de respeto de autoría del archivo base
st.markdown(
    """
